...
- **Side-effecting calls must be strict with ERA.** Any function that performs side effects (I/O, mutation, allocation) must be explicit about those effects and cannot rely on implicit ERA behavior. ERA interactions are structural — they erase and rewire, not "run" effects. Keep pure and effectful concerns separate.

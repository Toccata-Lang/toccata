...
**Side effects in lazy positions (open question).** Dropping a continuation's threaded value was observed to skip *earlier* side effects — only the last print appeared (plan, Verified facts; `scratch/rdr-tokens.toc` `read-and-show`). Intended vs. actual behavior undetermined; needs experiments. Working practice until resolved: thread an accumulator through every step so the final result depends on all of them.

**`strSha1` includes the value's TYPE in the hash.** `runtime3.c` `strSha1` does `Sha1Update(&arg0->type, 8)` before the bytes, so a `SubString` and a `String` with the same content hash **differently** — a hash map keyed by one type misses lookups with the other. They should hash to the same value; fix pending. Workaround until then: keep a map's keys a single type (the interpreter's parse output copies SubString keys to full Strings — `sub-to-str` in `interpreter/intrp-rdr.toc`).

...
Value *addCopiedBMI(BitmapIndexedNode *node, Term key, Term val, int64_t hash, int shift) {
  ...
  if (n >= 16) {
    ...
    for (int i = 0, j = 0; i < ARRAY_NODE_LEN; i++) {
      if ((node->bitmap >> i) & 1) {
	// theres something in this bit mapped position
	if (node->array[j] == 0) {
	  // it's sub node
...
Value *addMutateBMI(BitmapIndexedNode *node, Term key, Term val, int64_t hash, int shift) {
  int bit = bitpos(hash, shift);
  int idx = __builtin_popcount(node->bitmap & (bit - 1));
  ...
  if (n >= 16) {
    ...
    node->bitmap = 0;
    dec_and_free(termVal((Term)node), 1);
    return((Value *)newNode);
  } else {
    int itemCount = n + 1;
    BitmapIndexedNode *newNode = malloc_bmiNode(itemCount);
    ...
    newNode->array[2 * idx] = (Term)key;
    newNode->array[2 * idx + 1] = (Term)val;
    for (int i = idx * 2; i < n * 2; i++) {
      if (node->array[i] != 0) {
	incRef((Term)node->array[i], 1);
      }
...
void bmiSetKey(BitmapIndexedNode *node, int bit, Term key) {
  int idx = __builtin_popcount(node->bitmap & (bit - 1));
  node->array[2 * idx] = key;
}
...
void bmiSetVal(BitmapIndexedNode *node, int bit, Term val) {
  int idx = __builtin_popcount(node->bitmap & (bit - 1));
  node->array[2 * idx + 1] = val;
}
...
Value *bmiReplaceCopied(BitmapIndexedNode *node, Term key, Term val, int64_t hash, int shift, Term currKey, Term currVal) {
  ...
  // keys are not equal, how about the hashes
  int64_t existingKeyHash = sha1((FnArity *)0, incRef(currKey, 1));
  if (existingKeyHash == hash) {
    // hashes are equal at this level, so we need a hash collision node
    HashCollisionNode *newLeaf = malloc_hashCollisionNode(2);
    newLeaf->array[0] = (Value *)currKey;
    newLeaf->array[1] = (Value *)currVal;
    newLeaf->array[2] = (Value *)key;
    newLeaf->array[3] = (Value *)val;
    incRef(currKey, 1);
    incRef(currVal, 1);
...
Value *bmiReplaceMutate(BitmapIndexedNode *node, Term key, Term val, int64_t hash, int shift, Term currKey, Term currVal) {
  int bit = bitpos(hash, shift);
  int idx = __builtin_popcount(node->bitmap & (bit - 1));

  // there is already a key/val pair at the position where key
  // would be placed. Extend tree a level
  int64_t existingKeyHash = sha1((FnArity *)0, incRef((Term)currKey, 1));
  if (existingKeyHash == hash) {
...
Value *bmiCopyAssoc(BitmapIndexedNode *node, Term key, Term val, int64_t hash, int shift) {
  ...
  if (node->bitmap & bit) {
    ...
    Term currKey = bmiKey(node, bit);
    Term currVal = bmiVal(node, bit);
    if (equal(incRefVal(key, 1), incRefVal(currKey, 1))) {
      if (equal(incRefVal(val, 1), incRefVal(currVal, 1))) {
        dec_and_free(key, 1);
        dec_and_free(val, 1);
        return((Value *)node);
      } else {
...
Value *bmiMutateAssoc(BitmapIndexedNode *node, Term key, Term val, int64_t hash, int shift) {
  if (node->refs != 1) {
	...
	return bmiReplaceMutate(node, key, val, hash, shift, currKey, currVal);
      ...
      return addMutateBMI(node, key, val, hash, shift);
    }
  }
}
...
Value *copyAssoc(Value *node, Value *k, Value *v, int64_t hash, int shift) {
  switch(node->type) {
  case BitmapIndexedType:
    return(bmiCopyAssoc((BitmapIndexedNode *)node, (Term)k, (Term)v, hash, shift));
  case ArrayNodeType:
...
Value *mutateAssoc(Value *node, Value *k, Value *v, int64_t hash, int shift) {
  switch(node->type) {
  case BitmapIndexedType:
    return(bmiMutateAssoc((BitmapIndexedNode *)node, (Term)k, (Term)v, hash, shift));
  case ArrayNodeType:

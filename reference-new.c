...
void eraseLazy(Term lazyVar, Term era) {
  ...
  case DUP: {
    Location dupLoc = term_loc(negLaz);
    Term dup1 = get(port(1, dupLoc));
    Term dup2 = get(port(2, dupLoc));

    if (era == sideEffects) {
      forceLazy(laz);
    } else if (dup1 == laz) {
      // take the port that hasn't been erased
      take(port(2, dupLoc));
      pair_free(lazyLoc);
      swapStore(port(1, dupLoc), posLaz);
    } else if (dup2 == laz) {
      // take the port that hasn't been erased
      take(port(1, dupLoc));
      pair_free(lazyLoc);
      swapStore(port(2, dupLoc), posLaz);
    } else {
      print_term("laz", laz);

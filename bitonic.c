...
static inline Pair pop_redex(TM* tm) {
  if (tm->hput > 0) {
    ...
    return atomic_exchange_explicit(&globalNet->rbag_buf[tm->tid*(G_RBAG_LEN/TPC) + (--tm->rput)], 0, memory_order_relaxed);
  } else {
    return 0;
  }
}
...
bool CALL_down(TM *tm, Port a, Port b) {
  ...
  Port k4 = NONE;
  // fast anni
  if (get_tag(b) == CON && node_load(get_val(b)) != 0) {
    tm->itrs += 1;
    k1 = 1;
  ...
  Port k8 = NONE;
  // fast anni
  if (get_tag(k4) == CON && node_load(get_val(k4)) != 0) {
    tm->itrs += 1;
    k5 = 1;
  ...
  Port k12 = NONE;
  // fast anni
  if (get_tag(k8) == CON && node_load(get_val(k8)) != 0) {
    tm->itrs += 1;
    k9 = 1;
...
bool CALL_down__C0(TM *tm, Port a, Port b) {
  ...
  Port k16 = NONE;
  // fast anni
  if (get_tag(k12) == CON && node_load(get_val(k12)) != 0) {
    tm->itrs += 1;
    k13 = 1;
  ...
  Port k23 = NONE;
  // fast anni
  if (get_tag(k7) == CON && node_load(get_val(k7)) != 0) {
    tm->itrs += 1;
    k20 = 1;
...
bool CALL_sum(TM *tm, Port a, Port b) {
  ...
  Port k9 = NONE;
  // fast anni
  if (get_tag(k3) == CON && node_load(get_val(k3)) != 0) {
    tm->itrs += 1;
    k6 = 1;
...
bool CALL_warp__C1(TM *tm, Port a, Port b) {
  ...
  Port k20 = NONE;
  // fast anni
  if (get_tag(k16) == CON && node_load(get_val(k16)) != 0) {
    tm->itrs += 1;
    k17 = 1;
  ...
  Port k24 = NONE;
  // fast anni
  if (get_tag(k20) == CON && node_load(get_val(k20)) != 0) {
    tm->itrs += 1;
    k21 = 1;
  ...
  Port k28 = NONE;
  // fast anni
  if (get_tag(k19) == CON && node_load(get_val(k19)) != 0) {
    tm->itrs += 1;
    k25 = 1;
  ...
  Port k35 = NONE;
  // fast anni
  if (get_tag(k11) == CON && node_load(get_val(k11)) != 0) {
    tm->itrs += 1;
    k32 = 1;
...
static inline bool ERAS(TM* tm, Port a, Port b) {
  ...
  // Checks availability
  if (node_load(get_val(b)) == 0) {
    //printf("[%04x] unavailable0: %s\n", tid, show_port(b).x);
    return FALSE;
  }

  // Loads ports.
  Pair B  = node_exchange(get_val(b), 0);
  Port B1 = get_fst(B);
  Port B2 = get_snd(B);
...
static inline bool ANNI(TM* tm, Port a, Port b) {
  ...
  // Checks availability
  if (node_load(get_val(a)) == 0 || node_load(get_val(b)) == 0) {
...
void evaluator(TM* tm) {
  ...
  while (TRUE) {
    ...
    // If we have no redexes...
      ...
      Pair got = atomic_exchange_explicit(&globalNet->rbag_buf[idx], 0, memory_order_relaxed);
      if (got != 0) {
